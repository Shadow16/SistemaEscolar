...
{
	public partial class NuevoProfesor : Gtk.Window
	{
		private Cordinacion padre;
		public NuevoProfesor (Cordinacion padre) : 
		...
		protected void OnGuardarClicked (object sender, EventArgs e)
		{
			try 
			{
				...
				if(codigo == "" || nombre == "" || telefono == "" || apellidoP  == "" || apellidoM == "" || calle == "" || numero == "" || numeroInt == "" || 
					...
					this.abrirConexion();
					string sql = "INSERT INTO `Maestro` (`codigo`, `nombre`, `apellidoP`, `apellidoM`, `calle`, `numero`, `numeroInt`, `ciudad`, `colonia`, `telefono`, `avisarA`, `telEm`, `usuario`, `password`) " +
					             "VALUES ('" + codigo + "', '" + nombre + "', '" + apellidoP + "', '" + apellidoM + "', '" + calle + "', '" + numero + "', '" + numeroInt + "', '" + ciudad + "', '" + colonia + "', '" + telefono + "', '" + avisarA + "', '" + telEm + "', '" + usuario + "', '" + password + "')";
