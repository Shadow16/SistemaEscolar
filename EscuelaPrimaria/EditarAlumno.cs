...
{
	public partial class EditarAlumno : Gtk.Window
	{
		private Cordinacion padre;
		public EditarAlumno (Cordinacion padre) : 
		...
		protected void OnGuardarClicked (object sender, EventArgs e)
		{
			try 
			{
				...
				if(codigo == "" || nombre == "" || telefono == "" || apellidoP  == "" || apellidoM == "" || calle == "" || numero == "" || numeroInt == "" || 
					...
					this.abrirConexion();
					string sql = "UPDATE `Estudiante` SET `codigo`='" + codigo + "', `nombre`='" + nombre + "', `apellidoP`='" + apellidoP + "', " +
