...
{
	public partial class EditarProfesor : Gtk.Window
	{
		private Cordinacion padre;
		public EditarProfesor (Cordinacion padre) : 
		...
		protected void OnGuardarClicked (object sender, EventArgs e)
		{
			try 
			{
				...
				if(codigo == "" || nombre == "" || telefono == "" || apellidoP  == "" || apellidoM == "" || calle == "" || numero == "" || numeroInt == "" || 
					...
					this.abrirConexion();
					string sql = "UPDATE `Maestro` SET `codigo`='" + codigo + "', `nombre`='" + nombre + "', `apellidoP`='" + apellidoP + "', " +
					             ...
					             "`telEm`='" + telEm + "', `usuario`='" + usuario + "', `password`='" + password + "' WHERE (`id`='" + id + "')";
